...
export function TodoPage() {
  ...
  const handleAddTask = () => {
    ...
    addTask(selectedGroup, newTask, newStatus, isPriority);
    setNewTask("");
  ...
  return (
    <section className="flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between px-1">
        <PageBreadcrumb current="todo" currentLabel="TODO" />
      ...
      {showInput && (
        <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (name && name.trim()) {
                        ...
                        if (newId) setSelectedGroup(newId);
                      }
                      setShowDropdown(false);
                    }}
          ...
          {/* PRIORITY */}
          <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white">
            <input
              type="checkbox"
              checked={isPriority}
          ...
          <button
            ...
            className="rounded-xl bg-white/90 px-4 py-3 text-sm font-semibold text-black hover:bg-white"
          >
            Add
          </button>
        </div>
