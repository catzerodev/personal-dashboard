...
function MiniCard({ title, value }) {
  return (
    <div className="rounded-[24px] border border-white/10 bg-white/8 p-5">
      ...
      <p className="mt-3 text-4xl font-medium tracking-[-0.02em] text-white">{value}</p>
    </div>
  );
}
...
function SideCard({ title, children }) {
  return (
    <div className="rounded-[26px] border border-white/10 bg-white/8 p-6">
      <h3 className="mb-5 text-2xl font-medium tracking-[-0.02em] leading-[1.1] text-white">{title}</h3>
      {children}
    </div>
  );
}
